                    ...
                    <div class="ecom-swiper-wrapper ecom-product-single__media--images ecom-swiper-wrapper ecom-product-single__media--images-layout__${this.layout}">
                        {% for image in product.images%}
                            <div class="${e} ecom-product-single__media--image${this.enable_zoom?" ecom-image-zoom":""}" ${this.outerZoom?'data-ecom-zoom-layout="outer"':""}  data-index="{{forloop.index0}}"  data-variant_id="{{ image.variants | map:'id' | join: ','  }}">
                                ${this.enable_zoom?'<a href="{{ image | img_url: "2048x2048"}}" class="ecom-img-zoom-a" data-ecom-role="zoom-target">':""}
                                <img class="ecom-image-default${(W=this.data.settings)!=null&&W.disable_lazyload?"":" ecom-image-lazyload"}" ${this.image_action==="lightbox"?`    ecom-modal-source="{{image | img_url: 'master' }}" ecom-modal="image" `:""} src="{{ image | img_url: "2048x2048"}}" alt="{{ image.alt | escape }}" {% unless forloop.first %} {% unless EComBuilderMode %} ${(X=this.data.settings)!=null&&X.disable_lazyload?"":'loading="lazy"'} {% endunless %} {% endunless%} />
                                ${this.enable_zoom?"</a>":""}
                            </div>
                        {% endfor %}
                        {%- for media in product.media -%}
                            {% case media.media_type %}
                                ...
                                {% when 'video' %}
                                    <div data-stopdrag="true" class="${e} ecom-product-single__media--video ecom-product-single__media--full ecom-swiper-no-swiping" data-position="{{media.position}}" {% comment %}style="padding-top: {{ 1 | divided_by: media.aspect_ratio | times: 100 }}%;"{% endcomment %}>
                                        {{ media | video_tag: image_size:'master', class: 'media-video', controls: ${(D=this.data.settings)==null?void 0:D.video_control}, autoplay: ${(Y=this.data.settings)==null?void 0:Y.video_auto_play}, mute: ${(J=this.data.settings)==null?void 0:J.video_mute} }}
                                        ${(G=this.data.settings)!=null&&G.video_auto_play?"":`
                                                <button  class="ecom-product-single__media--play-control" type="button">
                                                    <span class="ecom-product-single__media--play-control-wrapper">
                                                        <span class="visually-hidden">Play video</span>
                                                        ${(U=this.data.settings)!=null&&U.video_icon?this.data.settings.video_icon.value:""}
                                                    </span>
                                                </button>
                                            `}
                                    </div>
                        ...
                        {% for image in product.images%}
                            <div class="ecom-product-single__media--thumbnail ecom-swiper-slide"  data-variant_id="{{ image.variants | map:'id' | join:','  }}">
                                <img
                                    ...
                                    alt="{{ image.alt  }}"
                                    ${(K=this.data.settings)!=null&&K.disable_lazyload?"":'loading="lazy"'}
                                />
                            </div>
                        {% endfor %}
                        {%- for media in product.media -%}
                                ...
                                <div class="ecom-product-single__media--thumbnail ecom-swiper-slide">
                                    <div class="ecom-product-single__media--thumbnail--icon">
                                        {% if media.media_type == 'model' %}
                                             ${(Q=this.data.settings)!=null&&Q.thumbnail_model_icon?this.data.settings.thumbnail_model_icon.value:""}
                                        {% else %}
                                            ${(ee=this.data.settings)!=null&&ee.thumbnail_video_icon?this.data.settings.thumbnail_video_icon.value:""}
                                        {% endif %}
                                    </div>
                                    <img src="{{ media.preview_image | img_url: '${this.thumbnail_size}' ${this.thumbnail_crop!=="none"?`, crop: '${this.thumbnail_crop}'`:""} }}"   alt="{{ media.alt }}" ${(te=this.data.settings)!=null&&te.disable_lazyload?"":'loading="lazy"'}/>
                                </div>
                            {% endif %}
                        {%- endfor -%}
                        </div>
